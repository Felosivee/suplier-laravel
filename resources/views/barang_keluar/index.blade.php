...
<div class="max-w-[720px] mx-auto py-32 sm:py-8 lg:py-16">
  ...
  <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
      <div>
          ...
          <p class="text-slate-500">A list of all the outgoing stock data..</p>
      </div>
      <div class="ml-3">
          ...
          <div class="relative">
              ...
              placeholder="Search for outgoing stock..."
              />
              <button
  ...
  <table class="w-full text-left table-auto min-w-max">
      ...
      <tr class="hover:bg-slate-50 border-b border-slate-200">
          ...
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $keluar->nama_barang }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $keluar->tgl_keluar }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $keluar->jml_keluar }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $keluar->lokasi }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $keluar->penerima }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500"><a href="{{ route('barang_keluar.edit', $keluar->id) }}" class="font-medium text-blue-600 hover:text-blue-800">Edit</a></p>
          </td>
          <td class="p-4 py-5">
           <p class="text-sm text-slate-500">
               <a href="{{ route('barang_keluar.destroy', $keluar->id) }}"
                   class="font-medium text-blue-600 hover:text-blue-800"
                   onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this item?')) { document.getElementById('delete-form-{{ $keluar->id }}').submit(); }">
                    Hapus
                </a>
                <form id="delete-form-{{ $keluar->id }}" action="{{ route('barang_keluar.destroy', $keluar->id) }}" method="POST" style="display: none;">
                   @csrf
                   @method('DELETE')
               </form>
           </p>
